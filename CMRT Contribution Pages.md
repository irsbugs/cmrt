Note: Contacts -> GDPR Dashboard

GDPR stands for General Data Protection Regulation, which is a comprehensive law that protects personal data and privacy for individuals within the European Union. It aims to give people more control over their personal information and establishes strict guidelines for how organizations handle such data.



# Manage Contribution Pages

Screenshots of *Manage Contribution Pages* as of 2026-03-05, using CiviCRM 6.10.2 used by Spark Essentials.

## Opening the Manage Contribution Pages main screen.

Administer --> CiviContribute -->  Manage Contibution Pages.

![mcp-01](/images/mcp/mcp-01.png)

## Configure
Current Contribution Pages:
* CMRT Donate
* CMRT Membership Signup Page

Each Contribution Page has a *Configure >* option which opens a drop-down menu of eight selectable items as follows:

1. Title and Settings. *Title* <-- This opens the *Contribution Links* page with 7 tab option.
2. Contribution Amounts. *Amounts* <-- All other options open *Contribution Links* page with 9 tabs.
3. Membership settings. *Memberships*
4. Thank-you and Receipting. *Receipt*
5. Include Profiles. *Profiles*
6. Contribution Widget. *Widgets*
7. Premiums. *Premuiums*
8. Personal Campaign Pages. *Personal Campaigns*

The nineth option that is available only as a tab selection is: *Terms and Conditions* 

The anomaly of 7 tabs for *Titles and Settings* compared with 9 tabs from selecting anoy of the other 8 options. 

![mc-01](/images/mcp/mc-01.png)
![mc-02](/images/mcp/mc-02.png)

*Configure* The following catagories may be configured:

![mcp-02](/images/mcp/mcp-02.png)
  
## 1. Title

Title and Settings for CMRT Donate. The *Title* tab.

![mcp-03](/images/mcp/mcp-03.png)

## 2. Amounts

Contribution Amounts for CMRT Donate

![mcp-04](/images/mcp/mcp-04.png)

## 3. Membership

Membership settings

![mcp-05](/images/mcp/mcp-05.png)

## 4. Receipt

Thank-you and Receipting

![mcp-06](/images/mcp/mcp-06.png)

## 5. Profile

Include Profiles

![mcp-07](/images/mcp/mcp-07.png)

## 6. Premiums

## 7. Widgets

Contribution Widget

![mcp-08](/images/mcp/mcp-08.png)

## 8. Personal Campaigns

TODO:

## 9. Terms and Conditions

![mcp-09](/images/mcp/mcp-09.png)
